...
function TrinningLandpage() {
  ...
  //       savedPosts[userId] = [];
  //     }
  //     const alreadySaved = savedPosts[userId].some((post) => post.jobId === jobId);
  //     if (!alreadySaved) {
  //       savedPosts[userId].push(jobPostToSave);
  //       localStorage.setItem("savedJobPosts", JSON.stringify(savedPosts));
  //       console.log("Job post saved successfully:", jobPostToSave);
  //     } else {
  ...
  const handleSave = async (jobPost) => {
    try {
      const userId = localStorage.getItem('userId'); // Retrieve user ID from local storage
      const response = await axios.post(
        `http://localhost:9090/prava/${userId}/saved-jobposts`, // Use the retrieved user ID
        jobPost
      );
      console.log(response.data);
      alert("Job Saved Successfully");
              ...
              >
                <div class="card bg-body">
                  ...
                  <div class="d-flex justify-content-between p-2">
                    <span className="">
                      <i class="fa-solid fa-graduation-cap text-warning"></i>{" "}
                      <b> PostedOn:</b> {jobPost.datePosted}
                    </span>

                    <span>
                      <i class="fa-solid fa-envelope text-warning"></i>{" "}
                      <b> Last Date: </b> {jobPost.applyLastDate}
                    </span>
                  ...
                  <div className="d-flex  p-2">
                    <span>
                      <i class="fa-solid fa-envelope text-warning"></i>{" "}
                      <b> Skills:</b> {jobPost.desiredSkillSet}
                    </span>
                  </div>
                  ...
                  <center>
                    <button
                      ...
                      id="applybutton"
                      className=" btn btn-warning border-success fw-bold  text-dark p-2 ms-5"
                    >
